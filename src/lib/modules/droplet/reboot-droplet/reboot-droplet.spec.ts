...
import * as MOCK from './reboot-droplet.mock';
...
describe('droplet', () => {
  ...
  describe('reboot-droplet', () => {
    ...
    it('should return IResponse<RebootDropletResponse>', async () => {
      const _rebootDroplet = rebootDroplet(context);
      const response = await _rebootDroplet({
        id: DROPLET_ID,
      });
      ...
      expect(requestBody.type).toBe('reboot');
      /// validate data

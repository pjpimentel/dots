...
import * as MOCK from './power-off-droplet.mock';
...
describe('droplet', () => {
  ...
  describe('reboot-droplet', () => {
    ...
    it('should return IResponse<PowerOffDropletResponse>', async () => {
      const _powerOffDroplet = powerOffDroplet(context);
      const response = await _powerOffDroplet({
        id: DROPLET_ID,
      });
      ...
      expect(requestBody.type).toBe('power_off');
      /// validate data

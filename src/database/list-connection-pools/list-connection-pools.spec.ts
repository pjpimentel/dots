...
describe('list-connection-pools', () => {
  ...
  it('should call axios.get', async () => {
    ...
    await _listConnectionPools(default_input);

    expect(httpClient.get).toHaveBeenCalledWith(`/databases/${default_input.database_cluster_id}/pools`, {
      params: {
        page: 1,
        per_page: 25,
      },
  ...
  it('should use `page` and `per_page` input', async () => {
    ...
    await _listConnectionPools(input);
  ...
  it('should output axios response', async () => {
    const _listConnectionPools = listConnectionPools(context);
    const output = await _listConnectionPools(default_input);
